...
(function($) {
    const methods = {
        init: function(options) {
            return this.each(function() {
                ...
                const $clickEle = $(self).find(self.opt.clickEle);
                const subMenuEle = self.opt.subMenuEle;
                const activeClass = self.opt.activeClass;
    ...
    $.fn.menuToggle = function(options) {
        if (methods[options]) {
            // 传进来的是字符串，检测是否存在该方法，存在则执行
            return methods[options].apply(this, Array.prototype.slice.call(arguments, 1));
        } else if (typeof options === 'object' || !options) {
            // 若是对象，或者不存在，则执行内部初始化init方法
